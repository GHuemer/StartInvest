...
class ProjectionBloc extends Bloc<ProjectionEvent, ProjectionState> {
  ...
  void _onAdd(AddProjectionAsset event, Emitter<ProjectionState> emit) {
    final alreadyAdded =
        state.assets.any((a) => a.ticker == event.input.ticker);
    if (alreadyAdded) return;
    emit(ProjectionSetup(
  ...
  void _onUpdateAmount(
      UpdateProjectionAmount event, Emitter<ProjectionState> emit) {
    emit(ProjectionSetup(
      assets: state.assets
          .map((a) =>
              a.ticker == event.ticker ? a.copyWith(amount: event.amount) : a)
          .toList(),
      periodMonths: state.periodMonths,
      periodLabel: state.periodLabel,
    ));
  ...
  void _onSelectPeriod(
      SelectProjectionPeriod event, Emitter<ProjectionState> emit) {
    emit(ProjectionSetup(
      assets: state.assets,
  ...
  Future<void> _onLoadHistory(
      LoadProjectionHistory event, Emitter<ProjectionState> emit) async {
    final result = await _repository.getProjectionHistory();
    result.fold(
      (Failure f) => emit(ProjectionError(
        assets: state.assets,
        periodMonths: state.periodMonths,
        periodLabel: state.periodLabel,
        message: f.message,
      )),
      (history) => emit(ProjectionHistoryLoaded(
        assets: state.assets,
        periodMonths: state.periodMonths,
        periodLabel: state.periodLabel,
        history: history,
  ...
  Future<void> _onRun(
      RunSimulation event, Emitter<ProjectionState> emit) async {
    ...
    result.fold(
      ...
      (simulation) => emit(ProjectionComplete(
        assets: state.assets,
        periodMonths: state.periodMonths,
        periodLabel: state.periodLabel,
        result: simulation,
